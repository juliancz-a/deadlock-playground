...
@export var size: float = 0.5:
	set(value):
		size = maxf(value, 0.0001)
		if camera:
			camera.size = size

## Corresponds to the far value of the underlying camera
@export var max_distance: float = 0.35:
	set(value):
		max_distance = clampf(value, 0.05, 100.0)
		if camera:
			max_distance = maxf(max_distance, camera.near + 0.01)
...
func _ready() -> void:
	...
	if get_viewport() is SubViewport:
		viewport.world_3d = (get_viewport() as SubViewport).find_world_3d()
	...
	camera.cull_mask = int(1) << int(20)  # Set layer 21 strictly and exclusively to detect brush render

	# Ensure Forward+ renderer allocates lighting cluster descriptors (Set 3) for the brush viewport
	if not camera.has_node("BrushSet3OmniLight"):
		...
		dummy_omni.shadow_enabled = false
		camera.add_child(dummy_omni)
	...
	viewport.screen_space_aa = SubViewport.SCREEN_SPACE_AA_DISABLED
	viewport.render_target_update_mode = SubViewport.UpdateMode.UPDATE_ALWAYS if drawing else SubViewport.UpdateMode.UPDATE_DISABLED
