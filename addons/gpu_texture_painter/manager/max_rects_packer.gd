...
# Public API
static func pack_into_square(rects: Array[Vector2], padding: int = 4) -> Array[Rect2]:
    # rects: Array of Vector2(width, height)
    # Returns: Array<Rect2> placements in normalized 0–1 coordinates

    var padded_rects: Array[Vector2] = []
    for r: Vector2 in rects:
        padded_rects.append(Vector2(r.x + padding * 2, r.y + padding * 2))

    var total_area: int = 0
    var max_side: int = 0
    for r: Vector2 in padded_rects:
    ...
    while low < high:
        ...
        if _can_pack(padded_rects, mid):
            high = mid
        else:
            low = mid + 1

    var final_size: int = low
    var placements: Array[Rect2] = _do_pack(padded_rects, final_size)

    # Normalize placements to 0–1 (insetting by padding so outer border has transparent gutter)
    var normalized: Array[Rect2] = []
    for r: Rect2 in placements:
        var pos: Vector2 = (r.position + Vector2(padding, padding)) / final_size
        var size: Vector2 = (r.size - Vector2(padding * 2, padding * 2)) / final_size
